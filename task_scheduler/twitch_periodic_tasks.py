...
def twitch_parse():
    ...
    try:
        with (ManagedFile(token_tw, "rb") as t,
        ...
        with (ManagedFile(token_tw, "wb") as new_token,
...
#        streamer_lists_online = [online['data'][n]['user_login'] for n in range(len(online['data']))]
        for k in range(len(online['data'])):
            try:
                writting_streamer_in_bd = Twitch_model.objects.create(streamer=online['data'][k]['user_login'])
