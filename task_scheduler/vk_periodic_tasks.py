...
def posts_vk_group():
    ...
    for sorted_all_respon in sorted_all_respons:
        try:
            link_post = 'http://' + sorted_all_respon['text'].split(': http://')[1].split('\n')[0]
        except IndexError:
            link_post = 'http://habr.com'
        try:
            link_image_post = sorted_all_respon['attachments'][0]['photo']['sizes'][-1]['url']
        except KeyError:
            link_image_post = 'No image'
        try:
            vk_published_post = Vk.objects.create(
                    data_post=datetime.fromtimestamp(sorted_all_respon['date']).strftime('%Y-%m-%d %H:%M:%S'),
