...
def youtube_parse():
    ...
    def foo_0(n):
        ...
        for x in range(n):
            newItemCount = response_subscriptions["items"][x]["contentDetails"]["newItemCount"]
            channelId = response_subscriptions["items"][x]["snippet"]["resourceId"]["channelId"]
            if newItemCount != 0:
    ...
    for playlistsId, newItemCount in lists_dicts_id_chs.items():
        ...
        try:
            sorted_list = sorted(videoId_videoPublishedAt, key=lambda d: d['videoPublishedAt'], reverse=True)
        except KeyError:
